...
using ModelFirstApp.Models;

namespace ModelFirstApp.Controllers
{
    public class Company_tblController : Controller
    {
        ...
        public ActionResult Edit([Bind(Include = "Id,Name,Age,Gender")] Company_tbl company_tbl)
        {
            if (ModelState.IsValid)
            {
                db.Entry(company_tbl).State = (System.Data.Entity.EntityState)System.Data.EntityState.Modified;
                db.SaveChanges();
                return RedirectToAction("Index");
            }
            return View(company_tbl);
